...
if __name__ == '__main__':
    """
        Главная функция для работы с конфигурацией программы
        Пример запуска >>> python config.py -a 950 750 1200 870 -p 50 55 -s 1000 1000 -t 0
        Параметры:
            --area  - рабочая область на фотографии
            --point - точка сварки на фотографии
            --size  - размер изображения в милиметрах (для определения масштаба)
            --type  - тип обработки изображения (вдоль перпендикуляра, в окружности, оба способа)
        Если параметры передавать при запуске файла main.py, то будут использоваться переданные,
        но конфиги не затираются
    """
